...
/// <summary>
/// Thin wrapper over the YouTube Data API read surface used by comment monitoring. Authorized per call
/// with a short-lived OAuth access token (<c>youtube.force-ssl</c>) resolved through the shared
/// <c>IGoogleChannelCredentials</c> contract for the account that owns the channel — the SAME credential
/// that authorizes moderation, so monitoring and reject are gated alike. A transient/quota failure
/// surfaces as the underlying <c>Google.GoogleApiException</c> so the caller can inspect the reason
/// (e.g. <c>quotaExceeded</c>, <c>commentsDisabled</c>) and react.
/// </summary>
public interface IYouTubeClient
{
    ...
    /// are flattened in with <see cref="YouTubeComment.IsReply"/> set. Authorized by
    /// <paramref name="accessToken"/>.
    /// </summary>
    Task<CommentFetchResult> GetRecentCommentsAsync(string accessToken, string youtubeChannelId, int maxResults = 50, CancellationToken ct = default);
    ...
    /// </summary>
    Task<VideoTitlesResult> GetVideoTitlesAsync(string accessToken, IEnumerable<string> videoIds, CancellationToken ct = default);
