    ...
    ILogger<UploadJobHandler> logger)
{
    ...
    public async Task RunAsync(Guid jobId, CancellationToken ct)
    {
        ...
        var tempDir = options.Value.TempDownloadDir;
        ...
        try
        {
            ...
            // Active project (= its own daily quota). One channel × N projects ⇒ N× the daily cap.
            ...
            var info = await drive.GetInfoAsync(driveService, job.DriveFileId, downloadCreds.ProjectId, ct);
            ...
            reservedProjectId = chosen.ProjectId;
        ...
        IReadOnlyList<GoogleUploadCreds> candidates, IQuotaService quota)
    {
        ...
        foreach (var c in candidates)
            ranked.Add((c, (await quota.GetStatusAsync(c.ProjectId)).RemainingUploads));
        ...
        foreach (var (creds, _) in ranked.OrderByDescending(r => r.remaining))
            if (await quota.TryReserveUploadAsync(creds.ProjectId))
                return creds;
        return null;
    }
