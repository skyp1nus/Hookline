    ...
    ILogger<SlackStatusService> logger) : ISlackStatusService
{
    ...
    {
        var botToken = await sp.GetRequiredService<SlackChannelService>().GetBotTokenForChannelAsync(channelId, ct);
        ...
        var tsKey = RedisKeys.StatusTs(channelId);
        ...
        if (repost)
        {
            ...
            if (newTs is not null) await db.StringSetAsync(tsKey, newTs, StatusTtl);
        ...
        else if (!string.IsNullOrEmpty(ts))
        {
            ...
            {
                await db.KeyExpireAsync(tsKey, StatusTtl); // refresh the key's lifetime on a live edit
            }
            else
            {
                var newTs = await slack.PostMessageAsync(botToken, channelId, text, blocks, ct: ct);
        ...
        IServiceProvider sp, string channelId, Guid? accountId, CancellationToken ct)
    {
        ...
        var google = sp.GetRequiredService<GoogleAccountsService>();
        var progress = sp.GetRequiredService<IProgressTracker>();

        var snap = await jobs.GetStatusSnapshotAsync(channelId, 5, ct);

        // Quota is keyed by PROJECT (Cloud project) and one YouTube channel can be consented through a
        // POOL of projects (each its own daily cap ⇒ N× quota), which is exactly what upload rotation uses.
        var effectiveAccountId = accountId ?? await google.GetDefaultAccountIdAsync(ct);
        string? channelTitle = null;
        IReadOnlyList<Guid> projectIds = Array.Empty<Guid>();
        if (effectiveAccountId is { } accId)
        {
            channelTitle = await google.GetAccountChannelLabelAsync(accId, ct);
            projectIds = await google.GetChannelProjectIdsAsync(accId, ct);
        }
    ...
    /// <summary>Sum a channel's per-project daily quota into one remaining/total uploads pair. Each id is a
    ...
    internal static async Task<(int RemainingUploads, int TotalUploads)> AggregateQuotaAsync(
        IReadOnlyList<Guid> projectIds, IQuotaService quota)
    {
