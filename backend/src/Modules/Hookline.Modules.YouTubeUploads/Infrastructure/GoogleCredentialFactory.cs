...
/// <summary>
/// Builds the OAuth pieces from an EXPLICIT client id + secret (one per Google Cloud project / "Project").
/// One consent covers both scopes. A <see cref="UserCredential"/> rebuilt from the stored refresh token +
/// its issuing client's secrets auto-refreshes its access token on every API call (no browser, suitable
/// for the background worker).
///
/// HARD RULE: a refresh token can only be refreshed by the client that issued it — callers MUST pass the
/// issuing project's creds (see <c>GoogleAccountBinding.ProjectId</c>), never a different project's.
/// </summary>
